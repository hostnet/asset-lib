...
class TreeWalkerTest extends TestCase
{
    public function testWalk(): void
    {
        ...
        $walker = new TreeWalker(function (DependencyNodeInterface $node) use (&$seen): void {
            $seen[] = $node;
        });
    ...
    public function testWalkEarlyStop(): void
    {
        $seen = [];
    ...
    public function testWalkSkipSubtree(): void
    {
        $root        = new RootFile(new File('foo'));
        $child       = new RootFile(new File('foo'));
        $grand_child = new RootFile(new File('foo'));
